...
# Function to check and assign oyster trends
checkOyTrends <- function(modelEstimate, lowConfidence, upConfidence){
  increasing <- modelEstimate > 0
  if(is.na(modelEstimate)){
    trendPresent <- "no model"
  } else {
    trendPresent <- ifelse(lowConfidence < 0 & upConfidence < 0, "yes", 
                           ifelse(lowConfidence > 0 & upConfidence > 0, "yes", "no"))    
  }
  trendStatus <- 0
  if(trendPresent=="yes"){
    trendStatus <- ifelse(increasing, 1, -1)
  } else if(trendPresent=="no") {
    trendStatus <- 0
  } else {
    trendStatus <- NA
  }
...
oyster_results %<>% rowwise() %>%
  mutate(numericTrend = checkOyTrends(ModelEstimate, LowerConfidence, UpperConfidence)) %>%
  as.data.table()
# Check and assign coral trends
  ...
  if(habitat=="CW"){
    if(nrow(subset)==0){
      ...
      for(group in unique(subset$LandCoverGroup)){
        ...
        if(ma=="Kristin Jacobs Coral Aquatic Preserve"){
          g_subset <- g_subset %>% filter(NumberOfYears>1)
        }
...
generate_disc_text <- function(data, indicator, ma){
  ...
  if(nrow(data)==0){
    ...
    n_parameters_display <- ifelse(n_parameters==1, glue("{n_parameters} parameter"), glue("{n_parameters} parameters"))
    if(indicator=="Nutrients"){
      ...
      sentence <- glue(
        "Water clarity {trended} between {time_period} based on an aggregate model of analyses results from {n_parameters_display}.")
      numericTrend <- unique(data$Trend)      
    }
  }
...
generate_sav_text <- function(data, ma){
  ...
  sentence <- stri_replace_all_regex(
    sentence,
    pattern = c("johnson's seagrass", "halophila spp.", "unidentified halophila", "total sav"),
    replacement = c("Johnson's seagrass", "<i>Halophila</i> spp.", "unidentified <i>Halophila</i>", "Total SAV"),
    vectorize = FALSE
  )
  # return(sentence)
  return(list("text" = sentence,
    ...
    if(coral_pc$N_Years<5){
      ...
      trend <- ifelse(str_detect(coral_pc$Trend, "increasing"), "an increase", 
                      ifelse(str_detect(coral_pc$Trend, "decreasing"), "a decrease", 
                             ifelse(str_detect(coral_pc$Trend, "No"), "no detectable change")))
...
generate_oyster_text <- function(ma, ind){
  ...
  habtype <- ifelse(ma_abrev %in% c("YRMAP"), "Restored", "Natural")
    ...
    if(nrow(oy_subset[SufficientData==FALSE]) == nrow(oy_subset)){
      ...
      if(ind=="Shell Height"){
        t_oy_subset <- oy_subset %>% 
          ...
          reframe(trend = ifelse(numericTrend>0, "an increase", ifelse(numericTrend<0, "a decrease", "no detectable change"))) %>%
          as.data.table()
        ...
        den_sentence <- glue("Live oyster density within {ma_short} showed {trend} between {minDate} and {maxDate} for {tolower(habtype)} reefs.")
...
textResultsFinal$Trend[(textResultsFinal$SufficientData==TRUE & is.na(textResultsFinal$Trend) & 
                          str_detect(textResultsFinal$Text, "no detectable change"))] <- 0
