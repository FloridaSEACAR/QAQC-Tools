...
## No model: A statistical model could not be fitted for {N} stations.
```
...
generate_wc_description <- function(data, parameter) {
  ...
  if(nrow(increasing)>0){
    ...
    if(singular){
      ...
      if(class(increasing$min_slope)=="character"){
        sentence <- sprintf("At %s program location, %s increased by %s %s per year.", 
                            english(increasing$n_stations), display_parameter, increasing$min_slope, units)        
      } else {
        sentence <- sprintf("At %s program location, %s increased by %.2f %s per year.", 
                            english(increasing$n_stations), display_parameter, increasing$min_slope, units)            
      }
      sentences <- c(sentences, sentence)
    } else {
      sentence <- sprintf("At %s program locations, %s increased between %.2f and %.2f %s per year.", 
  ...
  if(nrow(decreasing)>0){
    ...
    if(singular){
      ...
      sentence <- sprintf("At %s locations there was a decrease in %s between %.2f and %.2f %s per year.", 
  ...
  if(nrow(no_change)>0){
    ...
    if(singular){
      sentence <- sprintf("No detectable change in %s was observed at %s location.", 
                          display_parameter, english(no_change$n_stations))
    } else {
      sentence <- sprintf("No detectable change in %s was observed at %s locations.", 
                          display_parameter, english(no_change$n_stations))
    }
    sentences <- c(sentences, sentence)
  }
  ...
  if(nrow(insufficient)>0){
    ...
    if(singular){
      sentence <- sprintf("A statistical model could not be fitted for %s station.", 
                          english(insufficient$n_stations))
      sentences <- c(sentences, sentence)       
    } else {
      sentence <- sprintf("A statistical model could not be fitted for %s stations.", 
                          english(insufficient$n_stations))
      sentences <- c(sentences, sentence) 
    }
  }
...
for(ma in unique(cont$ManagedAreaName)){
  ...
  for(parameter in unique(filtered_cont$ParameterName)){
    ...
    description <- stri_replace_all_regex(
      description,
      pattern = c(" ph", "mg/l", "ntu ", " deg c ", "ppt ", " % per"),
      replacement = c(" pH", "mg/L", "NTU ", "&deg;C ", "PPT ", "% per"),
      vectorize = FALSE
    )
