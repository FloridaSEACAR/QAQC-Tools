...
paramSnippets <- setDT(read.xlsx("data/TableDesc-Context-Snippets.xlsx", sheet = 1))
maSnippets <- setDT(read.xlsx("data/TableDesc-Context-Snippets.xlsx", sheet = 2))
...
intro_options_disc <- function(param, managed_area){
  sample(c(
    ...
    paste0("Presented here is the result for ", param, " trends at the ", managed_area, "."),
    paste0("The following table highlights trends in ", param, " within the ", managed_area, "."),
    paste0("The analysis here tests for changes in ", param, " in the ", managed_area, " over the study period.")
  ),1)
}
...
trend_snippets <- function(trend, parameter, period, sample_count){
  ...
  if(trend=="no significant trend"){
    snip <- c(
      paste0("The data indicate ", trend, " in ", parameter, " from ", period, 
             ", based on the analysis of ", sample_count, " samples. "),
      paste0("An analysis over ", period, " revealed ", trend, " in ", parameter, 
             " levels, with ", sample_count, " samples contributing to this finding. "),
      paste0("Over the course of ", period, ", ", trend, " was detected in ", 
             parameter, ", supported by ", sample_count, " samples. "),
      paste0("There was ", trend, " in ", parameter, " from ", period, 
             ", according to data from ", sample_count, " samples. ")
    )
  } else {
    snip <- c(
      paste0("The data indicate a ", trend, " in ", parameter, " from ", period, 
             ", based on the analysis of ", sample_count, " samples. "),
      paste0("An analysis over ", period, " revealed a ", trend, " in ", parameter, 
             " levels, with ", sample_count, " samples contributing to this finding. "),
      paste0("Over the course of ", period, ", a ", trend, " was detected in ", 
             parameter, ", supported by ", sample_count, " samples. "),
      paste0("A ", trend, " in ", parameter, " was observed from ", period, 
             ", according to data from ", sample_count, " samples. ")
    )
  }
  return(paste(sample(snip,1)))
}

# Unique parameter-based snippets
paramContextSnippets <- function(param){
  snippets <- paramSnippets[Parameter==param, contextSnippet]
  sample(snippets,1)
}
