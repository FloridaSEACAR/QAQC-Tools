...
generate_sav_description <- function(data) {
  ...
  if (length(increasing) == 1) {
    s <- increasing[[1]]
    sentences <- c(sentences, sprintf("An annual increase in percent cover was observed in %s (%.1f%%).", s$species, s$slope))
  } else if (length(increasing) == 2) {
    parts <- sapply(increasing, function(s) sprintf("%s (%.1f%%)", s$species, s$slope))
    sentences <- c(sentences, sprintf("Annual increases in percent cover were observed in %s and %s.", parts[1], parts[2]))
  } else if (length(increasing) > 2) {
    parts <- sapply(increasing, function(s) sprintf("%s (%.1f%%)", s$species, s$slope))
    species_list <- paste(parts[1:(length(parts)-1)], collapse = ", ")
    species_list <- paste0(species_list, ", and ", parts[[length(parts)]])
    sentences <- c(sentences, sprintf("Annual increases in percent cover were observed in %s.", species_list))
  }
  
  ## Decreasing
  if (length(decreasing) == 1) {
    s <- decreasing[[1]]
    sentences <- c(sentences, sprintf("An annual decrease in percent cover was observed in %s (%.1f%%).", s$species, s$slope))
  } else if (length(decreasing) == 2) {
    parts <- sapply(decreasing, function(s) sprintf("%s (%.1f%%)", s$species, s$slope))
    sentences <- c(sentences, sprintf("Annual decreases in percent cover were observed in %s and %s.", parts[1], parts[2]))
  } else if (length(decreasing) > 2) {
    parts <- sapply(decreasing, function(s) sprintf("%s (%.1f%%)", s$species, s$slope))
    species_list <- paste(parts[1:(length(parts)-1)], collapse = ", ")
    species_list <- paste0(species_list, ", and ", parts[[length(parts)]])
    sentences <- c(sentences, sprintf("Annual decreases in percent cover were observed in %s.", species_list))
  }
  ...
  if (length(no_change) > 0) {
    ...
    if (length(no_change) == 1) {
      sentences <- c(sentences, sprintf("No detectable change in percent cover was observed in %s.", no_change[[1]]))
    } else if (length(no_change) == 2) {
      sentences <- c(sentences, 
                     sprintf("No detectable change in percent cover was observed in %s and %s.", 
  ...
  if (length(insufficient) > 0) {
    if (length(insufficient) == 1) {
      insufficient_sentence <- sprintf("Trends could not be evaluated for %s due to insufficient data", insufficient[1])
    } else if (length(insufficient) == 2) {
      insufficient_sentence <- sprintf("Trends could not be evaluated for %s and %s due to insufficient data", insufficient[1], insufficient[2])
    } else {
      ...
      insufficient_sentence <- sprintf("Trends could not be evaluated for %s due to insufficient data", species_list)
    }
  }
  
  if (length(model_not_fit) > 0) {
    if (length(model_not_fit) == 1) {
      no_model_sentence <- sprintf("model could not be fitted for %s.", model_not_fit[1])
    } else if (length(model_not_fit) == 2) {
      no_model_sentence <- sprintf("model could not be fitted for %s and %s.", model_not_fit[1], model_not_fit[2])
    } else {
      ...
      no_model_sentence <- sprintf("model could not be fitted for %s.", species_list)
    }
  }
  
  if(length(insufficient)>0 & length(model_not_fit)>0){
    combined_sentence <- paste0(insufficient_sentence, ", and a ", no_model_sentence)
    ...
    sentences <- c(sentences, paste0("A ",no_model_sentence))
  }
...
for(ma in unique(sav_lme$ManagedAreaName)){
  ...
  description <- stri_replace_all_regex(
    description,
    pattern = c("total sav", "Total sav", "halophila, unk.", "Halophila, unk.","halophila spp.", "Halophila spp."),
    replacement = c("total SAV", "Total SAV", "unknown <i>Halophila</i>", "unknown <i>Halophila</i>", "<i>Halophila</i> spp.", "<i>Halophila</i> spp."),
    vectorize = FALSE
  )
  ...
  descriptionText <- data.table(
    ...
    "Type"= "None",
    "ParameterName" = "Percent Cover (by species)",
