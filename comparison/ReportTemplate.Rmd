...
check_flag <- function(h){
  ...
  if(!h=="Species"){
    for(i in names(data_directory[[h]][["quantile"]])){
      data_combined_subset <- rbind(data_combined_subset, plyr::rbind.fill(data_directory[[h]][["quantile"]][[i]]), fill=TRUE)
    }
    ...
    for(type in names(data_directory[[h]][["quantile"]])){
      for(i in names(data_directory[[h]][["quantile"]][[type]])){
        data2 <- plyr::rbind.fill(data_directory[[h]][["quantile"]][[type]][[i]])
        data2$habitat <- type
