...
author: "(SIDBGD) SEACAR Internal Division for the Betterment and Goodification of Data"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
for(i in 1:length(habitats)){
  ...
  if(nrow(data_directory[[h]][["program_count_table"]])>0){
    ...
    cat("  \n")
    cat("Values in red represent data that was removed or re-classified. Values in green represent data that has been added or 
        re-classified [where nOld or nNew is NA] (either due to a change in the Include value or a new Program). ProgramName values in blue
        represent programs which are managed by RCP and *currently collecting data*.")
    cat("  \n")
    program_count_table <- data_directory[[h]][["program_count_table"]] %>%
      rowwise() %>%
      mutate(cell_color = ifelse(is.na(LatestDateOld) | is.na(LatestDateNew), "green", ifelse(difference<0, "red", "black")),
             prog_color = ifelse(str_detect(ProgramName, "APCWQM|NERR") & year(LatestDateNew) >= year(Sys.Date())-2, "blue", "black")) %>%
      mutate(difference = cell_spec(difference, color=cell_color),
             ProgramName = cell_spec(ProgramName, color = prog_color)) %>%
      select(-c(cell_color, prog_color))
  ...
  if(h %in% c("Species", "Discrete WQ")){
    ...
    ### Observe changes in Jan dates between exports
    # Load in previous Jan dates data to compare with current
    available_data <- list.files("output/rds/", full.names = T, pattern = paste0("jan_dates_", h))
    file_info <- file.info(available_data) # grab file info to determine most recent
    most_recent <- rownames(file_info)[which.max(file_info$mtime)]
    old_jan_dates <- readRDS(most_recent) %>% rename("n_old" = "n", "years_old" = "years")
    # Combine new and old jan dates files to perform comparison
    combined_jan_dates <- jan_dates_display %>% merge(old_jan_dates) %>%
      dplyr::rowwise() %>%
      dplyr::mutate(cell_color_n = ifelse(n==n_old, "black", "orange"),
                    cell_color_years = ifelse(years==years_old, "black", "orange")) %>%
      dplyr::mutate(n = cell_spec(n, color = cell_color_n),
                    years = cell_spec(years, color = cell_color_years)) %>%
      select(-c(n_old, years_old, cell_color_n, cell_color_years))
    # Save a copy of January dates to observe changes between exports 
    # (must be saved after loading "most_recent" or else it will be loaded as the most recent)
    saveRDS(jan_dates_display, paste0("output/rds/jan_dates_", h, "_", Sys.Date(), ".rds"))
    ...
    cat("  \n")
    cat("  \n")
    cat("A comparison is performed between the current export and the previous export. If the number (n) of values has changed or a 
        difference in years is detected, the values in the *n* column or the *years* column will be highlighted in orange.")
    cat("  \n")
    print(
      make_kable(
        combined_jan_dates, 
  ...
  # Filter out empty data frames and then combine them
  non_empty_fut_dfs <- Filter(function(df) nrow(df) > 0, data_directory[[h]][["future_dates"]])
  future_dates <- bind_rows(non_empty_fut_dfs)
  ...
  if(nrow(future_dates)>0){
    ...
    cat("## Future Dates")
    cat("  \n")
    cat(colorize("FUTURE DATES FOUND", "red"))
    cat("  \n")
    print(
      make_kable(
        future_dates %>% group_by(Habitat, ProgramID, ProgramName, Year, Month, ParameterName) %>% reframe(n = n()),
        table_caption = "Summary of Future Dates",
        format = "latex"
      )
    )
  } else {
    ...
    cat("## Future Dates")
    cat("  \n")
    cat(glue("No future dates found for {h}."))
    cat("  \n")
    cat("  \n")
  }
  ...
  if(h=="Species"){
    cat("  \n")
    cat("## Oyster UniversalReefID")
    oyster_urid <- data_directory[[h]]$OysterReefID %>% 
      bind_rows() %>%
      mutate(ProgramLocationID = gsub("_", "-", ProgramLocationID))
    if(nrow(oyster_urid)>0){
      cat("  \n")
      cat(colorize("LocationIDs without UniversalReefID values found!", "red"))
      cat("  \n")
      print(
        make_kable(
          oyster_urid, 
          table_caption = "Summary of locations missing UniversalReefIDs",
          format = "latex"
        )
      )
      cat("  \n")
      ...
      cat("  \n")
      cat("All Oyster LocationIDs have assigned UniversalReefID values.")
      cat("  \n")
    }
    cat("  \n")
  }
